...
function InvoiceForm({ invoice, token, onSubmit, onCancel }) {
  ...
  const validateOwnCompanyTab = () => {
    const errors = []
    if (!formData.companyName) errors.push('Company Name')
    if (!formData.companyAddress) errors.push('Company Address')
    if (!formData.companyState) errors.push('Company State')
    return errors
  }
  ...
  const validateClientTab = () => {
    ...
    if (!formData.clientAddress) errors.push('Client Address')
    if (!formData.clientState) errors.push('Client State')
    return errors
  }

  const validateInvoiceTab = () => {
    const errors = []
    if (!formData.number) errors.push('Invoice Number')
    if (!formData.invoiceDate) errors.push('Invoice Date')
    if (!formData.dueDate) errors.push('Due Date')
    if (!formData.poNumber) errors.push('PO Number')
    return errors
  }
  ...
  const handleNextTab = (e) => {
    ...
    switch (activeTab) {
      case 'owncompany':
        errors = validateOwnCompanyTab()
        break
      case 'client':
        errors = validateClientTab()
        break
      case 'invoice':
  ...
  return (
    <div className="form-overlay">
      <div className="form-container form-container-large" style={{ position: 'relative' }}>
        ...
        <div className="tabs" style={{ display: 'flex', gap: '4px', marginBottom: '1rem', borderBottom: '2px solid #e2e8f0', flexWrap: 'wrap' }}>
          <button
            type="button"
            className={`tab-btn ${activeTab === 'owncompany' ? 'active' : ''}`}
            onClick={() => setActiveTab('owncompany')}
            style={{
              padding: '0.75rem 1.5rem',
              border: 'none',
              background: activeTab === 'owncompany' ? '#7c3aed' : 'transparent',
              color: activeTab === 'owncompany' ? '#fff' : '#64748b',
              cursor: 'pointer',
              fontWeight: '600',
              borderBottom: activeTab === 'owncompany' ? '3px solid #7c3aed' : 'none',
              marginBottom: '-2px'
            }}
          >
            Own Company
          ...
          <button
            ...
            onClick={() => {
              const errors = validateOwnCompanyTab()
              if (errors.length > 0) {
                alert(`Please fill in the following mandatory fields:\n${errors.join('\n')}`)
                return
              }
            ...
            style={{
              padding: '0.75rem 1.5rem',
              border: 'none',
              background: activeTab === 'client' ? '#7c3aed' : 'transparent',
              color: activeTab === 'client' ? '#fff' : '#64748b',
              cursor: 'pointer',
              fontWeight: '600',
              borderBottom: activeTab === 'client' ? '3px solid #7c3aed' : 'none',
              marginBottom: '-2px'
            }}
          >
            Client Details
          ...
          <button
            ...
            onClick={() => {
              const errors = validateClientTab()
              if (errors.length > 0) {
                alert(`Please fill in the following mandatory fields:\n${errors.join('\n')}`)
                return
              }
            ...
            style={{
              padding: '0.75rem 1.5rem',
              border: 'none',
              background: activeTab === 'invoice' ? '#7c3aed' : 'transparent',
              color: activeTab === 'invoice' ? '#fff' : '#64748b',
              cursor: 'pointer',
              fontWeight: '600',
              borderBottom: activeTab === 'invoice' ? '3px solid #7c3aed' : 'none',
              marginBottom: '-2px'
            }}
          >
            Invoice Details
          ...
          <button
            ...
            style={{
              padding: '0.75rem 1.5rem',
              border: 'none',
              background: activeTab === 'status' ? '#7c3aed' : 'transparent',
              color: activeTab === 'status' ? '#fff' : '#64748b',
              cursor: 'pointer',
              fontWeight: '600',
              borderBottom: activeTab === 'status' ? '3px solid #7c3aed' : 'none',
              marginBottom: '-2px'
            }}
          >
            Status
        ...
        <form id="invoice-form" onSubmit={handleSubmit}>
          ...
          {activeTab === 'client' && (
            <fieldset style={{ border: '1px solid #ccc', marginBottom: 16, padding: 12 }}>
              ...
              <div className="form-row">
                <div className="form-group">
                  <label htmlFor="clientGSTN">Client GSTN</label>
                  <input
                    type="text"
                    id="clientGSTN"
                    name="clientGSTN"
          ...
          <div className="form-actions">
            ...
            {activeTab === 'status' ? (
              ...
              <button type="button" className="btn btn-primary" onClick={(e) => handleNextTab(e)}>
                {getButtonText()}
              </button>
            )}
          </div>
